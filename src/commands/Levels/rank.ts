...
export class RankCommand extends Command {
	...
	private async buildCard(message: Message, user: User, type: 'voice' | 'text'): Promise<void> {
		...
		let info: VoiceExperience | TextExperience | null =
			type === 'voice'
		...
		const requiredXP =
			type === 'voice'
				? this.container.helpers.leveling.xp.experienceFormula(level + 1)
				: this.container.helpers.leveling.xp.textExperienceFormula(level + 1);
