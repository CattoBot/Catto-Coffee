...
export class BanReasonModalHandler extends InteractionHandler {
    ...
    public override parse(interaction: ModalSubmitInteraction) {
        if (interaction.customId !== 'user-ban') return this.none();
        return this.some();
    }

    public async run(interaction: ModalSubmitInteraction): Promise<InteractionResponse> {
        const name = interaction.fields.getTextInputValue('usr-ban');
