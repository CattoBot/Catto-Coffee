...
export class ChannelOwnerPrecondition extends Precondition {
	...
	private async checkChannelOwnership(member: GuildMember, context: CommandInteraction | Message) {
		...
		if (!getChannel) {
			return this.error({
				message: await resolveKey(context, 'commands/replies/commandDenied:voice_not_found_in_db', { emoji: Emojis.ERROR })
			});
		}

		if (member.id !== getChannel.channelOwnerId && !isTrustedUser) {
			return this.error({
				message: await resolveKey(context, 'commands/replies/commandDenied:not_voice_channel_owner', { emoji: Emojis.ERROR })
			});
		}
