...
export class RoleCommandPermitPrecondition extends Precondition {
    ...
    private async runChatInput(interaction: ChatInputCommandInteraction) {
        ...
        if (missingRoles.length === 0) {
            ...
            const messageContent = await resolveKey(interaction, `preconditions/preconditions:MISSING_ROLE`, { roles: `\`${missingRoleNames.join(", ")}\``, emoji: Emojis.ERROR });
    ...
    private async runMessage(message: Message) {
        ...
        if (missingRoles.length === 0) {
            ...
            const messageContent = await resolveKey(message, `preconditions/preconditions:MISSING_ROLE`, { roles: `\`${missingRoleNames.join(", ")}\``, emoji: Emojis.ERROR });
            return this.error({ message: messageContent });
        }
    }
