...
export class ProfileCardBuilder {
	...
	private async drawBadges() {
		...
		if (userBadges.length > 0) {
			...
			this.ctx.quadraticCurveTo(
				...
				this.BADGE_CONTAINER_Y,
				containerX + containerWidth,
				this.BADGE_CONTAINER_Y + this.BADGE_CONTAINER_RADIUS
			);
			this.ctx.lineTo(containerX + containerWidth, this.BADGE_CONTAINER_Y + containerHeight - this.BADGE_CONTAINER_RADIUS);
			this.ctx.quadraticCurveTo(
				containerX + containerWidth,
				this.BADGE_CONTAINER_Y + containerHeight,
				containerX + containerWidth - this.BADGE_CONTAINER_RADIUS,
				this.BADGE_CONTAINER_Y + containerHeight
			);
			...
			this.ctx.quadraticCurveTo(
				...
				this.BADGE_CONTAINER_Y + containerHeight,
				containerX,
				this.BADGE_CONTAINER_Y + containerHeight - this.BADGE_CONTAINER_RADIUS
			);
	...
	private drawRank() {
		...
		this.ctx.quadraticCurveTo(
			this.RANK_CONTAINER_X + rankContainerWidth,
			this.RANK_CONTAINER_Y,
			this.RANK_CONTAINER_X + rankContainerWidth,
			this.RANK_CONTAINER_Y + this.RANK_CONTAINER_RADIUS
		);
		...
		this.ctx.quadraticCurveTo(
			this.RANK_CONTAINER_X + rankContainerWidth,
			this.RANK_CONTAINER_Y + rankContainerHeight,
			this.RANK_CONTAINER_X + rankContainerWidth - this.RANK_CONTAINER_RADIUS,
			this.RANK_CONTAINER_Y + rankContainerHeight
		);
		this.ctx.lineTo(this.RANK_CONTAINER_X + this.RANK_CONTAINER_RADIUS, this.RANK_CONTAINER_Y + rankContainerHeight);
		this.ctx.quadraticCurveTo(
			this.RANK_CONTAINER_X,
			this.RANK_CONTAINER_Y + rankContainerHeight,
			this.RANK_CONTAINER_X,
			this.RANK_CONTAINER_Y + rankContainerHeight - this.RANK_CONTAINER_RADIUS
		);
		this.ctx.lineTo(this.RANK_CONTAINER_X, this.RANK_CONTAINER_Y + this.RANK_CONTAINER_RADIUS);
		this.ctx.quadraticCurveTo(
			this.RANK_CONTAINER_X,
			this.RANK_CONTAINER_Y,
			this.RANK_CONTAINER_X + this.RANK_CONTAINER_RADIUS,
			this.RANK_CONTAINER_Y
		);
		this.ctx.closePath();
		this.ctx.fillStyle = '#EAEAEA';
		...
		this.ctx.fillText(
			rankText,
			this.RANK_CONTAINER_X + rankContainerWidth / 2,
			this.RANK_CONTAINER_Y + rankContainerHeight / 2 + this.RANK_TEXT_Y_OFFSET
		);
	}
