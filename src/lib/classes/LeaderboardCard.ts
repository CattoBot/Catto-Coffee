...
export class LeaderboardImageBuilder {
	...
	private async drawLeaderboard(context: CanvasRenderingContext2D, top10: LeaderboardUserData[]): Promise<void> {
		...
		lb.forEach((user, index) => {
			...
			container.helpers.canvas.drawProgressBar(
				context,
				progressBarX,
				progressBarY,
				progressBarWidth,
			...
			if (this.showMessages) {
				...
				context.fillStyle = '#000000';
				context.fillText(
					`Messages: ${container.utils.numbers.format(messages ?? 0)}`,
					textX + MESSAGES_POSITION_X_OFFSET,
					progressBarY + MESSAGES_POSITION_Y_OFFSET
				);
			}
			if (this.showDailyTimeInVoiceChannel) {
				...
				context.fillStyle = '#000000';
				context.fillText(
					`Daily Time: ${secondsToHours(dailyTime ?? 0)}h`,
					textX + HOURS_POSITION_X_OFFSET,
					progressBarY + HOURS_POSITION_Y_OFFSET
				);
			}
			if (this.showWeeklyTimeInVoiceChannel) {
				...
				context.fillStyle = '#000000';
				context.fillText(
					`Weekly Time: ${secondsToHours(weeklyTime ?? 0)}h`,
					textX + HOURS_POSITION_X_OFFSET,
					progressBarY + HOURS_POSITION_Y_OFFSET
				);
			}
			if (this.showMonthlyTimeInVoiceChannel) {
				...
				context.fillStyle = '#000000';
				context.fillText(
					`Monthly Time: ${secondsToHours(monthlyTime ?? 0)}h`,
					textX + HOURS_POSITION_X_OFFSET,
					progressBarY + HOURS_POSITION_Y_OFFSET
				);
			}
			...
			if (this.showMonthlyMessages) {
				...
				context.fillStyle = '#000000';
				context.fillText(
					`Monthly Messages: ${monthlyMessages}`,
					textX + MESSAGES_POSITION_X_OFFSET,
					progressBarY + MESSAGES_POSITION_Y_OFFSET
				);
			}
	...
	private async drawUserSection(
		context: CanvasRenderingContext2D,
		userId: string,
		userRank: number,
		userdata: LeaderboardUserData,
		avatarloadimage: Image
	): Promise<void> {
		...
		container.helpers.canvas.drawUserData(
			context,
			await container.client.users.fetch(userId).then((user) => user.username),
			container.utils.numbers.format(userdata[`${this.type}Level`] ?? 0),
			container.utils.numbers.format(userdata[`${this.type}Experience`] ?? 0),
			userDataX,
			userDataY
		);
		...
		container.helpers.canvas.drawProgressBar(
			...
			progressBarForUserX,
			progressBarForUserY,
			progressBarForUserWidth,
			progressBarForUserHeight,
			progressForUser,
			userColor.start,
			userColor.end
		);
		...
		if (this.showHours) {
			...
			context.fillStyle = '#000000';
			context.fillText(
				`Hours: ${secondsToHours(hours ?? 0)}h`,
				userDataX + HOURS_POSITION_X_OFFSET + 5,
				progressBarForUserY + HOURS_POSITION_Y_OFFSET
			);
		}
		if (this.showMessages) {
			...
			context.fillText(
				`Messages: ${container.utils.numbers.format(messages ?? 0)}`,
				userDataX + MESSAGES_POSITION_X_OFFSET,
				progressBarForUserY + MESSAGES_POSITION_Y_OFFSET
			);
		}
		if (this.showDailyTimeInVoiceChannel) {
			...
			context.fillText(
				`Daily Time: ${secondsToHours(dailyTime ?? 0)}h`,
				userDataX + HOURS_POSITION_X_OFFSET,
				progressBarForUserY + HOURS_POSITION_Y_OFFSET
			);
		}
		if (this.showWeeklyTimeInVoiceChannel) {
		...
		if (this.showWeeklyMessages) {
			...
			context.fillStyle = '#000000';
			context.fillText(
				`Weekly Messages: ${weeklyMessages}`,
				userDataX + MESSAGES_POSITION_X_OFFSET,
				progressBarForUserY + MESSAGES_POSITION_Y_OFFSET
			);
		}
	...
	public async build(): Promise<Buffer | null> {
		...
		const [avatarloadimage, backgroundImage] = await Promise.all([
			this.userId
				? loadImage(await container.client.users.fetch(this.userId).then((user) => user.displayAvatarURL({ extension: 'jpg', size: 256 })))
				: null,
