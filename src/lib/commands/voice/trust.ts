	...
	public static async messageRun(message: Message, args: Args) {
		...
		try {
			...
			try {
				...
				await message.reply({
					content: await resolveKey(message, 'commands/replies/commandDenied:voice_user_not_found')
				});
				return;
			}
			...
			if (message.member!.user.id !== owner.id) {
				return message.reply({
					content: await resolveKey(message, 'commands/replies/commandDenied:only_vc_owner')
				});
			}
			...
			if (message.member!.user.id === user.id) {
				return message.reply({
					content: await resolveKey(message, 'commands/replies/commandDenied:self_voice_command', { emoji: Emojis.ERROR })
				});
			}

			const trustedUser = await container.helpers.voice.findTrusted(owner.voice.channel!.id, message.guild!.id, user.id);

			if (trustedUser) {
				return message.reply({
					content: await resolveKey(message, 'commands/replies/voice:already_trusted', { user: user.displayName, emoji: Emojis.ERROR })
				});
			} else {
				try {
					await container.helpers.voice.store(owner.voice.channelId!, user.id, message.guild!.id);
					return message.reply({
						content: await resolveKey(message, 'commands/replies/voice:trust_success', { user: user.displayName, emoji: Emojis.SUCCESS })
					});
				} catch (err) {
					console.error('Error storing trusted user:', err);
					return message.reply({
						content: 'An error occurred while trying to store the trusted user. Please try again later.'
					});
				}
			}
		} catch (error) {
	...
	public static async chatInputRun(interaction: Subcommand.ChatInputCommandInteraction): Promise<InteractionResponse> {
		...
		if (interaction.user.id !== owner.id) {
			return await interaction.reply({
				content: translateKey('commands/replies/commandDenied:only_vc_owner'),
				ephemeral: true
			});
		}
		...
		const trustedUser = await container.helpers.voice.findTrusted(owner.voice.channel!.id, interaction.guild!.id, member.id);
		...
		if (trustedUser) {
			...
			await container.helpers.voice.store(owner.voice.channelId!, member.id, interaction.guild!.id).catch((err) => {
				console.log(err);
			});
