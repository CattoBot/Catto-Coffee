...
export class VoiceNameCommand {
	public static async messageRun(message: Message, args: Args): Promise<void> {
		...
		await message.member?.voice.channel?.setName(name.toString()).catch(async () => {
			await message.reply({
				content: await resolveKey(message, 'commands/replies/voice:voice_name_error', { emoji: Emojis.ERROR })
			});
			return;
		});
		await this.updateName(name, message.member!);
		await message.reply({
			content: await resolveKey(message, 'commands/replies/voice:voice_name_success', { emoji: Emojis.SUCCESS, name: name })
		});
	}
