	...
	public static async messageRun(message: Message, args: Args) {
		...
		if (member!.voice.channel!.members.has(user.id)) {
			return await message.reply(
				translateKey('commands/replies/voice:user_already_in_channel', { user: user.displayName, emoji: Emojis.WARN })
			);
		}
		...
		try {
			await Promise.all([
				...
				user
					.send(translateKey('commands/replies/voice:invite_dm', { guild: message.guild!.name, channel: member!.voice.channel!.url }))
					.catch(() => {
	...
	public static async chatInputRun(interaction: Subcommand.ChatInputCommandInteraction): Promise<InteractionResponse> {
		...
		if (member!.voice.channel!.members.has(interaction.user.id)) {
			return await interaction.reply({
				content: translateKey('commands/replies/voice:user_already_in_channel', { user: interaction.user.displayName, emoji: Emojis.WARN }),
				ephemeral: true
			});
		}
		...
		await Promise.all([
			...
			await user
				?.send({
					content: translateKey('commands/replies/voice:invite_dm', { guild: interaction.guild!.name, channel: member!.voice.channel!.url })
				})
				.catch(() => {
					return interaction.reply({
						content: translateKey('commands/replies/commandDenied:invite_dm_failed', { emoji: Emojis.ERROR }),
						ephemeral: true
					});
				})
		]);
		...
		return interaction.reply({
			content: translateKey('commands/replies/voice:invite_success', { user: user.displayName, emoji: Emojis.SUCCESS })
		});
	}
}
