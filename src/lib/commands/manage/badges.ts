	...
	public static async chatInputRun(interaction: Subcommand.ChatInputCommandInteraction) {
		...
		if (guild && guild.length > 0) {
			return interaction.editReply({
				content: await resolveKey(interaction, `commands/replies/error:already_badges`, { emoji: Emojis.ERROR })
			});
		}
		...
		try {
			...
			return interaction.editReply({
				content: await resolveKey(interaction, `commands/replies/admin:badge_success`, { emoji: Emojis.SUCCESS })
			});
		} catch (error) {
			console.error(error);
	...
	public static async chatInputRemove(interaction: Subcommand.ChatInputCommandInteraction) {
		...
		if (!guild || guild.length === 0) {
			return interaction.reply({
				content: await resolveKey(interaction, `commands/replies/error:no_badges`, { emoji: Emojis.ERROR }),
				ephemeral: true
			});
		}
		try {
			...
			return interaction.reply({
				content: await resolveKey(interaction, `commands/replies/admin:badge_remove`, { emoji: Emojis.SUCCESS }),
				ephemeral: false
			});
		} catch (error) {
			console.error(error);
			return interaction.reply({
				content: await resolveKey(interaction, `commands/replies/error:error`, { emoji: Emojis.ERROR }),
				ephemeral: true
			});
		}
	}
