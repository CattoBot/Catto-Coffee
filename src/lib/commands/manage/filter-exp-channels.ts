...
export class FilterVoiceChannelCommand {
    public static async add(interaction: Subcommand.ChatInputCommandInteraction): Promise<InteractionResponse> {
        ...
        if (result) {
            ...
            return await interaction.reply({
                content: await resolveKey(interaction, `commands/replies/admin:${module.toLowerCase()}_filter_channel_add`, { channel: channel, emoji: Emojis.SUCCESS }),
                ephemeral: false
            });
        }
    ...
    public static async remove(interaction: Subcommand.ChatInputCommandInteraction) {
        ...
        const channelExists = await this.channelExists(interaction.guild!.id, channel.id, module);
        if (!channelExists) {
            return await interaction.reply({ content: await resolveKey(interaction, `commands/replies/error:not_filtered_channel`), ephemeral: true });
        }
        ...
        return await interaction.reply({
            content: await resolveKey(interaction, `commands/replies/success:voice_bonus_channel_remove`, { channel: channel, emoji: Emojis.SUCCESS }),
            ephemeral: false
        });
    }
