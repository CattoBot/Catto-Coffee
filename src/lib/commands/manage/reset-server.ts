...
export class ResetServerCommand {
	...
	private static async resetModule(interaction: Subcommand.ChatInputCommandInteraction<CacheType>, moduleName: string) {
		if (interaction.channel?.isSendable()) {
			...
			const confirm = await interaction.channel
				?.awaitMessages({
			...
			if (!confirm || !['confirm', 'confirmar'].includes(confirm.first()?.content.toLowerCase() || '')) {
				return interaction.followUp({
					content: await resolveKey(interaction, `commands/replies/error:no_time_reset_server`, { emoji: Emojis.WARN }),
					ephemeral: false
				});
			}

			await interaction.editReply({
				content: await resolveKey(interaction, `commands/replies/admin:resetting_module`, { module: moduleName, emoji: Emojis.LOADING })
			});
			...
			await interaction.editReply({
				content: await resolveKey(interaction, `commands/replies/admin:reset_user_complete_emoji`, { emoji: Emojis.SUCCESS })
			});

			await interaction.followUp({
				content: await resolveKey(interaction, `commands/replies/admin:reset_complete`, { module: moduleName, emoji: Emojis.SUCCESS })
			});
			return;
		}
		return;
