...
export class AddUserToGuildBlacklistCommand {
    public static async run(interaction: Command.ChatInputCommandInteraction) {
        ...
        if (isBlacklisted || user) {
            const message = await resolveKey(interaction, `commands/replies/error:already_backlisted_user`, { emoji: Emojis.ERROR, user: fetchedUser.displayName });
        ...
        return await interaction.reply({
            embeds: [new Embed(`An error occurred while adding the user \`${fetchedUser.displayName}\` to the blacklist on the server \`${interaction.guild!.name}\`.`)]
        });
    }
