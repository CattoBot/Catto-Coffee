...
export class RemoveUserFromGuildBlacklistCommand {
	public static async run(interaction: Command.ChatInputCommandInteraction) {
		...
		if (!isBlacklisted || !user) {
			const message = await resolveKey(interaction, `commands/replies/error:user_not_blacklisted`, {
				emoji: Emojis.ERROR,
				user: fetchUser.displayName
			});
			await interaction.reply({
				embeds: [new Embed(message)]
			});
		} else {
			const message = await resolveKey(interaction, `commands/replies/admin:user_removed_from_blacklist`, {
				user: userId,
		...
		return await interaction.reply({
			embeds: [
				new Embed(`An error occurred while removing the user \`${userId}\` from the blacklist on the server \`${interaction.guild!.name}\`.`)
			]
		});
	}
