...
export class VoiceRoleCommands {
		...
		if (interaction.channel?.isSendable()) {
			...
			if (existingRole?.level === level) {
				return interaction.reply({
					content: await resolveKey(interaction, 'commands/replies/error:voice_role_exists', { level: level, emoji: Emojis.ERROR }),
					ephemeral: true
				});
			}
	...
	public static async remove(interaction: Subcommand.ChatInputCommandInteraction) {
		...
		return interaction.reply({
			content: await resolveKey(interaction, 'commands/replies/admin:voice_role_remove', { role: role, emoji: Emojis.SUCCESS })
		});
	}
}
