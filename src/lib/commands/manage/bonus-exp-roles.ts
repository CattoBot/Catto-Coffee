...
export class BonusVoiceRolesCommand {
	public static async add(interaction: Subcommand.ChatInputCommandInteraction) {
		...
		if (result) {
			return await interaction.reply({
				content: await resolveKey(interaction, `commands/replies/admin:${module.toLowerCase()}_bonus_role_add`, {
					role: `${role}`,
	...
	public static async remove(interaction: Subcommand.ChatInputCommandInteraction) {
		...
		if (result) {
			return await interaction.reply({
				content: await resolveKey(interaction, `commands/replies/admin:${module.toLowerCase()}_bonus_role_remove`, {
					role: role,
					emoji: Emojis.SUCCESS
				}),
				ephemeral: false
