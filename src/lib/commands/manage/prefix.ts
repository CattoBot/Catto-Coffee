...
export class PrefixCommand {
    public static async chatInputRun(interaction: Subcommand.ChatInputCommandInteraction) {
        ...
        return await interaction.reply({ content: await resolveKey(interaction, `commands/replies/admin:prefix_update`, { prefix: prefix, emoji: Emojis.SUCCESS }), ephemeral: false });
    }
    ...
    public static async messageRun(message: Message, args: Args) {
        ...
        if (prefix.length > 5) return message.reply({ content: await resolveKey(message, `commands/replies/error:prefix_length`, { emoji: Emojis.ERROR }) });
        await container.prisma.guilds.update({
            where: {
        ...
        return await message.reply({ content: await resolveKey(message, `commands/replies/admin:prefix_update`, { prefix: prefix, emoji: Emojis.SUCCESS }) });
    }
}