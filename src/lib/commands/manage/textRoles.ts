...
export class TextRoleCommands {
		...
		if (interaction.channel?.isSendable()) {
			...
			if (existingRole?.level === level) {
				return interaction.reply({
					content: await resolveKey(interaction, 'commands/replies/error:text_role_exists', { level: level, emoji: Emojis.ERROR }),
					ephemeral: true
				});
			}
	...
	public static async remove(interaction: Subcommand.ChatInputCommandInteraction) {
		...
		return await interaction.reply({
			content: await resolveKey(interaction, `commands/replies/admin:text_role_remove`, { role: role, emoji: Emojis.SUCCESS }),
			ephemeral: false
		});
	}
}
