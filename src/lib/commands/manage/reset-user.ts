	...
	public static async chatInputRun(interaction: Subcommand.ChatInputCommandInteraction<CacheType>) {
		...
		if (user.bot) {
			return interaction.reply({
				content: await resolveKey(interaction, 'commands/replies/error:bot_experience', { emojis: Emojis.ERROR }),
				ephemeral: true
			});
		}
		...
		if (module === 'Voice') {
			...
			return interaction.reply({
				content: await resolveKey(interaction, 'commands/replies/error:invalid_module', { emoji: Emojis.ERROR }),
				ephemeral: true
			});
		}
		return;
	...
	private static async resetModule(
		...
		if (interaction.channel?.isSendable()) {
			...
			await interaction.reply({
				content: await resolveKey(interaction, `commands/replies/admin:confirm_reset_user`, {
			...
			await interaction.editReply({
				content: await resolveKey(interaction, `commands/replies/admin:resetting_user_module`, {
					user: user.displayName,
					module: moduleName,
			...
			await interaction.followUp({
				content: await resolveKey(interaction, `commands/replies/admin:reset_user_complete`, {
					user: user.displayName,
					module: moduleName,
